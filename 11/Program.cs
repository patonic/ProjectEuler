...
namespace _11
{
    ...
    {
        static int numMax = 4;
        //' ' между числами, 'N' между рядами, матрица должна быть квадратная
        ...
        static void calculating()
        {
            ...
            int max = -1;
            Parallel.For(0, 4, (int method, ParallelLoopState pls) =>
             {
                 switch (method)
                 {
                     case 0:    //горизонтальный
                         {
                             Parallel.For(0, sequence.Count, (int y) =>
                               {
                                   Parallel.For(0, sequence.Count - (numMax - 1), (int x) =>
                                       {
                                           int num = sequence[y][x];
                     ...
                     case 1:    //вертикальный
                         {
                             Parallel.For(0, sequence.Count, (int x) =>
                             {
                                 Parallel.For(0, sequence.Count - (numMax - 1), (int y) =>
                                 {
                                     int num = sequence[y][x];
                                     for (int i = 1; i < numMax; i++)
                                         num *= sequence[y + i][x];
                     ...
                     case 2:    //горизонатльный \
                         ...
                         break;
                     case 3:    //горизонатльный /
                         {
                             Parallel.For(0, sequence.Count - (numMax - 1), (int y) =>
                             {
                                 Parallel.For(0, sequence.Count - (numMax - 1), (int x) =>
                                 {
                                     int num = sequence[y + (numMax - 1)][x];
                                     for (int i = 1; i < numMax; i++)
                                     {
                                         int tY = y + ((numMax - 1)-i);
                                         int tX = x + i;
